...
![](figure09.png){fig-align="center"}

## Sensitivity Analyses (Supp. Table 8-9)

**요약**: Main 분석결과를 뒷받침 하는 경향

* (**2회이상 복용조건**) 모든 신경정신질환 HR≥1.00
  + 지적장애가 가장 높고(HR 1.20, CI 1.13-1.28),
  + 자폐증이 가장 낮음(HR 1.07, CI 1.01-1.14).

* (**기저질환 자녀제외**) 모든 신경정신질환 HR≥1.00
  + ADHD(HR 1.14, CI 1.11-1.16) 및 중증 신경정신장애(HR 1.14, CI 1.08-1.20)가 가장 높고, 
  + 자폐증이 가장 낮음(HR 1.04, CI 1.00-1.09).
...
* 약제별 Subgroup 분석결과는 대체로 유사
  + H2 수용체 길항제는 강박장애 위험을 제외하고 p<.05 수준에서 HR≥1.00

* 임신기간별[^4] 분석결과, 대체로 임신초기 복용 시 위험이 높음
  + 대체로 임신 초기 복용시 p<.05 수준에서 HR≥1.00
  + 한편 강박장애는 임신 말기 복용시 오히려 감소(HR 0.89, CI 0.72-1.10)

[^4]: 마지막 생리일로부터 0~90일(first), 91~180일(second), 181일~출산(third)

## Sibling-Matched Analysis (Figure 4)

* 같은 어머니에게서 태어난, 노출여부가 다른 형제자매 분석
  + Main Analyses에서는 복용-발생위험 간 연관성이 나타났었음
  + Sibling-Matched 분석(어머니 고정효과?)에서는 연관이 사라짐

![](figure10.png){fig-align="center"}

## Discussion

* 대규모 전체 코호트에서는 작은 연관성이 관찰되었으나, 형제자매 비교분석에서는 사라짐.
  + 이전 동물실험(모체 장내 미생물군과 태아의 신경발달손상) 연구결과와 상반
  + 조산아의 위산억제제 사용과 신경정신질환 위험증가 연구결과와도 상반

* 가족 요인에 의한 잔여 교란요인 가능성이 큼.
...
1. 약을 처방받았다고 해서 반드시 먹은건 아님.\
  처방이 조제-복용으로 반드시 이어진다는 보장 없음.

2. Stratified Cox model(어머니 고정효과)로 미관측 교란요인을 통제했지만,\
...
[^5]: 한편 한국에선 규제수준이 높고 Universal Coverage 건강보험이 있어서 OTC 영향력은 낮을 것으로 예상